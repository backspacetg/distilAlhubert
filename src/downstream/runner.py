...
class Runner():
    ...
    def _get_downstream(self):
        expert = importlib.import_module(f"src.downstream.{self.args.downstream}.expert")
    ...
    def train(self):
        ...
        if self.config.get('specaug'):
            ...
            logging.info("[Runner]: adding specaug")

        freeze_finetune_steps = 0

        if self.config["runner"].get("freeze_finetune_steps") is not None:
            freeze_finetune_steps = int(self.config["runner"].get("freeze_finetune_steps"))
            logging.info(f"[Runner]: set freeze_finetune_steps to {freeze_finetune_steps}; upstream trainable: {self.upstream.trainable}")
        ...
        while pbar.n < pbar.total:
            ...
            for batch_id, (wavs, *others) in enumerate(tqdm(dataloader, dynamic_ncols=True, desc='train', file=tqdm_file)):
                ...
                try:
                    ...
                    if self.upstream.trainable and global_step > freeze_finetune_steps:
                        # print(f"[runner] start update upstream, {global_step} {freeze_finetune_steps}")
                        features = self.upstream.model(wavs)
                    else:
                        # print("[runner] do not update upstream")
                        with torch.no_grad():
                            features = self.upstream.model(wavs)
                    ...
                    if 'CUDA out of memory' in str(e):
                        logging.warning(f'[Runner] - CUDA out of memory at step {global_step}')
                        if is_initialized():
                            raise
                        with torch.cuda.device(self.args.device):
                            torch.cuda.empty_cache()
                        # self.downstream.zero_grad()
                        optimizer.zero_grad()
                        continue
                    else:
                        raise
                ...
                if math.isnan(grad_norm):
                    logging.warning(f'[Runner] - grad norm is NaN at step {global_step}')
                else:
                    optimizer.step()
                # self.downstream.zero_grad()
                optimizer.zero_grad()
    ...
    def evaluate(self, split=None, logger=None, global_step=0):
        ...
        for batch_id, (wavs, *others) in enumerate(tqdm(dataloader, dynamic_ncols=True, desc=split, total=evaluate_steps)):
            ...
            with torch.no_grad():
                ...
                batch_ids.append(batch_id)
        # with open(os.path.join(self.args.expdir, "evaluate_record.json"), 'w') as j:
        #     json.dump(records, j, ensure_ascii=False)

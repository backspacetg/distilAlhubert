...
# upstream rate是上游降采样率 audio到feature的
class SimpleLinear(nn.Module):
    ...
    def __init__(self, 
        ...
        **kargs):
        ...
        real_sample_rate = 1 if total_rate == -1 else round(total_rate / upstream_rate)
        if real_sample_rate > 1 and downsample_style == "concat":
            input_size = input_size * real_sample_rate
            print(f"[asr/model.py]: sample rate {real_sample_rate}, new input size {input_size}")
        
        self.final_proj = nn.Linear(in_features=input_size, out_features=output_size)

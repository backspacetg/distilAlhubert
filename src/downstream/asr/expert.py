...
class DownstreamExpert(nn.Module):
    ...
    def __init__(self, upstream_dim, upstream_rate, downstream_expert, expdir, **kwargs):
        ...
        logging.info("[asr expert] I am Here")
        ...
        self.use_projector = bool(self.modelrc.get("pre_downstream_projector", True))
        if self.use_projector:
            logging.info("[asr expert]: use projector")
            self.projector = nn.Linear(upstream_dim, self.modelrc['project_dim'])
        
        model_cls = eval(self.modelrc['select'])
        model_conf = self.modelrc[self.modelrc['select']]
        self.model = model_cls(
            self.modelrc['project_dim'] if self.use_projector else upstream_dim,
    ...
    def log_records(self, split, records, logger, global_step, batch_ids, total_batch_num, **kwargs):
        ...
        logging.info(f'{split} loss: {loss}')
        ...
        logging.info(f'{split} uer: {uer}')
        logging.info(f'{split} wer: {wer}')

...
class Wav2Vec2Model(nn.Module):
    ...
    def forward(
        ...
        x, layer_results = self.encoder(x, padding_mask=padding_mask, layer=layer, ret_attn=ret_attn)
    ...
    def extract_features(self, source, padding_mask, mask=False, layer=None, ret_attn=False):
        res = self.forward(
            source, padding_mask, mask=mask, features_only=True, layer=layer, ret_attn=ret_attn
        )
        return res
...
class TransformerEncoder(nn.Module):
    ...
    def forward(self, x, padding_mask=None, layer=None, ret_attn=False):
        x, layer_results = self.extract_features(x, padding_mask, layer, ret_attn=ret_attn)

        # print("final output: ", x.mean(), x.std())
    ...
    def extract_features(
        ...
        for i, layer in enumerate(self.layers):
            ...
            if not self.training or (dropout_probability > self.layerdrop):
                # print(f"input of layer {i}", x[:5,0,0])
                x, (z, lr) = layer(
                    x, self_attn_padding_mask=padding_mask, need_weights=ret_attn #HACK: add an option to make the wav2vec2 encoder to return weights
                )
                # print(f"output of layer {i}", x[:5,0,0])
                if i >= min_layer:
        ...
        if pad_length > 0:
            ...
            def undo_pad(a, b, c):
                return (
                    a[:-pad_length],
                    b[:, :-pad_length, :-pad_length] if b is not None else b,
                    c[:-pad_length],
                )

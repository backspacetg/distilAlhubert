...
class HubertModel(torch.nn.Module):
    ...
    def forward_targets(
        ...
        # feat2tar_ratio: 1.0 for hubert base
        # 对特征进行截断
    ...
    def forward(
        ...
        ret_dict = {"x": x, "padding_mask": padding_mask, "features": features, "layer_results": layer_results}
        if features_only:
            if ret_attn: 
                last_attn = layer_results[-1][1] #HACK: get attention form layer_results
                ret_dict["last_attn"] =  last_attn
            return ret_dict
        ...
        if not self.skip_masked: 
            masked_indices = torch.logical_and(~padding_mask, mask_indices) # 没有被padding掉 并且被Mask了的
            # 两个Mask都是False表示留
            # 布尔索引：在索引维度会被拉平
        ...
        ret_dict["features_pen"] = features_pen
        ret_dict["mask_indices"] = mask_indices #HACK: for distillation
    ...
    def extract_features(
        ...
        # print("hubert model", res["last_attn"])
        if ret_attn:
            return feature, res["padding_mask"], res["last_attn"]
        else:

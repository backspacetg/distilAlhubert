...
class UpstreamExpert(UpstreamBase):
    def __init__(self, ckpt, **kwargs):
        ...
        # print(kwargs["feature_selection"])
        if len(self.hooks) == 0 and kwargs["feature_selection"] != "no_feat": #HACK: disable hooking
            # self.add_hook("self.model.encoder", lambda input, output: output[1][-1][0])
    ...
    def forward(self, wavs, return_attn=False):
        ...
        if return_attn:
            features, feat_padding_mask, last_attn = self.model.extract_features(
                padded_wav,
                padding_mask=wav_padding_mask,
                mask=None,
                ret_attn=return_attn
            )
            return {"last_attn": last_attn} #HACK: return the last attention
        else:

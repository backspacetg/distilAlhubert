...
class Runner():
    ...
    def _get_upstream(self):
        ...
        module_path = f'src.pretrain.{self.args.upstream}.pretrain_expert'
    ...
    def train(self):
        ...
        # set amp
        amp = bool(self.config['runner'].get('fp16', False))
        print(self.config['runner'])
        if amp:
        ...
        while pbar.n < pbar.total:
            for data in tqdm(dataloader, dynamic_ncols=True, desc='train'):
                ...
                if global_step % self.config['runner']['save_step'] == 0 or pbar.n == pbar.total -1:
                    ...
                    self.check_ckpt_num(self.args.expdir)
                    ...
                    if pbar.n == pbar.total -1 and n_epochs > 0:
                        save_names.append(f"epoch-{n_epochs}")
                    else:
                        save_names.append(f"{global_step}")
                    if eval_loss is not None:
                        save_names.append("{:.8f}".format(eval_loss))
                        self.logger.add_scalar(f'{self.args.upstream}/eval-loss', eval_loss, global_step=global_step)
                    save_names = "-".join(save_names)
                    name = f'states-{save_names}.ckpt'
                    save_path = os.path.join(self.args.expdir, name)
                    
                    tqdm.write(f'[Runner] - Saving the checkpoint to: {save_path}')
    ...
    def evaluate(self):
        self.upstream.eval()
        # prepare data
        eval_batch_size = self.config['pretrain_expert']['datarc'].get('eval_batch_size', None)
        if eval_batch_size is None:
            tqdm.write("[Runner] - skip eval")
            self.upstream.train()
            return None
        dataloader = self.upstream.get_eval_dataloader()
        total_eval_steps = len(dataloader.dataset)
        ...
        # set progress bar
        pbar = tqdm(total=total_eval_steps, dynamic_ncols=True, desc='overall')
        ...
        for data in tqdm(dataloader, dynamic_ncols=True, desc='train'):
            ...
            # record loss
            total_avg_loss = (total_avg_loss * pbar.n + loss.item())/global_step
            del loss
            pbar.update(1)
        pbar.close()
        self.upstream.train()
    ...
    def check_ckpt_num(self, directory):
        max_keep = self.config['runner']['max_keep']
        ckpt_pths = glob.glob(f'{directory}/states-*.ckpt')
        if len(ckpt_pths) >= max_keep:
            eval_batch_size = self.config['pretrain_expert']['datarc'].get('eval_batch_size', None)
            if eval_batch_size is None:
                # save name: "exp/finetune/r9_100k_100h/states-200000.ckpt"
                ckpt_pths = sorted(ckpt_pths, key=lambda pth: int(pth.split('-')[-1].split('.')[0]))
            else:
                # save_name: "exp/finetune/r9_100k_100h/states-200000-15.39.ckpt"
                ckpt_pths = sorted(ckpt_pths, key=lambda pth: float(pth.replace(".ckpt", "").split('-')[-1]), reverse=True)

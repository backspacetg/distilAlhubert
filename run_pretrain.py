...
import numpy as np
#-------------#
from src.pretrain.runner import Runner
from src.utility.helper import override
...
def get_pretrain_args():
    ...
    # upstream settings
    parser.add_argument('-u', '--upstream', choices=os.listdir('src/pretrain/'))
    ...
    if args.past_exp:
        ...
        upstream_dirs = [u for u in os.listdir('src/pretrain/') if re.search(f'^{u}_|^{u}$', args.upstream)]
        assert len(upstream_dirs) == 1, upstream_dirs
...
if __name__ == '__main__':
    ...
    logging.basicConfig(level=logging.INFO, stream=sys.stdout, format=LOG_FORMAT, datefmt=DATE_FORMAT)
    main()
